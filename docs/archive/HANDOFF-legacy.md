> # ⚠️ HISTORICAL DOCUMENT — NOT A BACKLOG, NOT CURRENT
>
> This is the handoff as it stood **before** 2026-08-23. It is kept because it
> explains *why* much of the code looks the way it does. It is **not** a list of
> open work, and its 🔴 P0 / P1 markers are **not** current priorities.
>
> It predates the touch-control layer, the shadow warm-up fix, the GitHub Pages
> deployment and the custom domain. It still describes Pages as a future goal,
> and its front page opens "Nothing here is started" — which was true then and is
> false now. At least two of its playtest items (the duplicated menu title, the
> oil-slick roadblock) are already fixed in `main`.
>
> **For the current state, read [`../../PROJECT_STATE.md`](../../PROJECT_STATE.md).**
> Reproduce anything from this file against current `main` before treating it as
> work. A legacy document is evidence of what was believed then, not that an item
> is open now.
>
> *This banner is the only edit. Everything below is the original file verbatim;
> the untouched original also remains in git at `627401c:HANDOFF.md`.*
